...
d3.select("#dc_t3").append("svg")
    .attr("width", width)
    .attr("height", height)
    .attr("id", "dc-t3")
    .attr("class", "dc-t3");

var svg_dc_t3 = d3.select("#dc-t3");

var force_dc_t3 = d3.layout.force()
    .size([width, height]);

d3.select("#dc_t3")
  .append("div")
  .attr("id", "tooltip-dc-t3")
  .attr("class", "tooltip-dc-t3")
    .style("opacity", 0);
  
var tooltip_dc_t3 = d3.select("#tooltip-dc-t3");
    
d3.csv("graphs/DC/data/DC_T3_SNA_deidentified.csv", function(error, links) {
  ...
  var link = svg_dc_t3.selectAll(".link-dc-t3")
      .data(rels)
    .enter().append("line")
      .attr("class", "link-dc-t3")
      .on('mouseover.tooltip', function(d) {
      	tooltip_dc_t3.transition()
        	.duration(300)
        	.style("opacity", .8);
      	tooltip_dc_t3.html("Source: "+ d.source.name + 
	    ...
	    .on("mousemove", function() {
	      tooltip_dc_t3.style("left", (d3.event.pageX) + "px")
	        .style("top", (d3.event.pageY + 10) + "px");
      });
      ;

  // Create the node circles.
  var node = svg_dc_t3.selectAll(".node-dc-t3")
      .data(nodes)
    .enter().append("circle")
      .attr("class", "node-dc-t3")
      .attr("r", 4.5)
      .call(force_dc_t3.drag)
      .on('mouseover.tooltip', function(d) {
        tooltip_dc_t3.transition()
          .duration(300)
          .style("opacity", .8);
        tooltip_dc_t3.html("ID:" + d.name)
          .style("left", (d3.event.pageX) + "px")
          .style("top", (d3.event.pageY + 10) + "px");
      })

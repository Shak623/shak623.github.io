...
d3.select("#nj_t3").append("svg")
    .attr("width", width)
    .attr("height", height)
    .attr("class", "nj-t3")
    .attr("id", "nj-t3");

var svg_nj_t3 = d3.select("#nj-t3");

var force_nj_t3 = d3.layout.force()
    .size([width, height]);

d3.select("#nj_t3")
  .append("div")
  .attr("id", "tooltip-nj-t3")
	.attr("class", "tooltip-nj-t3")
    .style("opacity", 0);

var tooltip_nj_t3 = d3.select("#tooltip-nj-t3");

d3.csv("graphs/NJ/data/NJ_T3_SNA_deidentified.csv", function(error, links) {
  ...
  var link = svg_nj_t3.selectAll(".link-nj-t3")
      .data(rels)
    .enter().append("line")
      .attr("class", "link-nj-t3")
      .on('mouseover.tooltip', function(d) {
        tooltip_nj_t3.transition()
          .duration(300)
          .style("opacity", .8);
        tooltip_nj_t3.html("Source: "+ d.source.name + 
      ...
      .on("mousemove", function() {
        tooltip_nj_t3.style("left", (d3.event.pageX) + "px")
          .style("top", (d3.event.pageY + 10) + "px");
      });
      ;

  // Create the node circles.
  var node = svg_nj_t3.selectAll(".node-nj-t3")
      .data(nodes)
    .enter().append("circle")
      .attr("class", "node-nj-t3")
      .attr("r", 4.5)
      .call(force_nj_t3.drag)
      .on('mouseover.tooltip', function(d) {
        tooltip_nj_t3.transition()
          .duration(300)
          .style("opacity", .8);
        tooltip_nj_t3.html("ID:" + d.name)
          .style("left", (d3.event.pageX) + "px")
          .style("top", (d3.event.pageY + 10) + "px");
      })

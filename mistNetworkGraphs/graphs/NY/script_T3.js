...
d3.select("#ny_t3").append("svg")
    .attr("width", width)
    .attr("height", height)
    .attr("class", "ny-t3")
    .attr("id", "ny-t3");

var svg_ny_t3 = d3.select("#ny-t3");

var force_ny_t3 = d3.layout.force()
    .size([width, height]);

d3.select("#ny_t3")
  .append("div")
  .attr("id", "tooltip-ny-t3")
	.attr("class", "tooltip-ny-t3")
    .style("opacity", 0);

var tooltip_ny_t3 = d3.select("#tooltip-ny-t3");

d3.csv("graphs/NY/data/NY_T3_SNA_deidentified.csv", function(error, links) {
  ...
  var link = svg_ny_t3.selectAll(".link-ny-t3")
      .data(rels)
    .enter().append("line")
      .attr("class", "link-ny-t3")
      .on('mouseover.tooltip', function(d) {
        tooltip_ny_t3.transition()
          .duration(300)
          .style("opacity", .8);
        tooltip_ny_t3.html("Source: "+ d.source.name + 
      ...
      .on("mousemove", function() {
        tooltip_ny_t3.style("left", (d3.event.pageX) + "px")
          .style("top", (d3.event.pageY + 10) + "px");
      });
      ;

  // Create the node circles.
  var node = svg_ny_t3.selectAll(".node-ny-t3")
      .data(nodes)
    .enter().append("circle")
      .attr("class", "node-ny-t3")
      .attr("r", 4.5)
      .call(force_ny_t3.drag)
      .on('mouseover.tooltip', function(d) {
        tooltip_ny_t3.transition()
          .duration(300)
          .style("opacity", .8);
        tooltip_ny_t3.html("ID:" + d.name)
          .style("left", (d3.event.pageX) + "px")
          .style("top", (d3.event.pageY + 10) + "px");
      })
